...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
        ...
        /// </summary>
        public void HandleActionSellItem(uint vendorGuid, List<ItemProfile> itemProfiles, bool playerConfirmed = false)
        {
            if (IsBusy)
            {
                Session.Network.EnqueueSend(new GameEventInventoryServerSaveFailed(Session, Guid.Full));
            ...
            // convert a portion of the payout into MMD notes (Dynamic Vendor only)
            var payoutMmdAmount = (int)Math.Round(payoutCoinAmount / 287500.0, MidpointRounding.ToZero);

            if (sellList.Values.Any(x => x.ItemType == ItemType.PromissoryNote))
                payoutMmdAmount = 0;
            ...
            if (!playerConfirmed && vendor.DynamicVendorInfo != null)
            {
                ...
                ConfirmationManager.EnqueueSend(new Confirmation_Custom(Guid, () => HandleActionSellItem(vendorGuid, itemProfiles, true)), msg);
                Session.Network.EnqueueSend(new GameEventInventoryServerSaveFailed(Session, Guid.Full));
                SendUseDoneEvent();
                return;
            }
            ...
            itemsToReceive.Add(mmdStackWeenieClassId, payoutMmdAmount);
        ...
        private Dictionary<uint, WorldObject> VerifySellItems(List<ItemProfile> sellItems, Vendor vendor)
        {
            ...
            foreach (var sellItem in sellItems)
            {
                ...
                if (((acceptedItemTypes & wo.ItemType) == 0 || !wo.IsSellable || wo.Retained) && vendor.WeenieClassId != 21747013 /* Vendor is Bag of Foy */)
                {
