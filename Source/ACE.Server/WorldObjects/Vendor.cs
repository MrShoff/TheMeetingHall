...
namespace ACE.Server.WorldObjects
{
    ...
    public class Vendor : Creature
    {
            ...
            uint weenieClassId, int? palette, float? shade, int? stackSize)
        {
            ...
            {
                if (ModdedWeenies.JimineysSalePrices.TryGetValue(weenieClassId, out int specialPrice))
                {
            ...
            {
                var curItemInfo = DynamicVendorInfo.ItemValues.Find(x => x.Wcid == weenieClassId);
                if (curItemInfo != null)
                {
        ...
        public bool BuyItems_ValidateTransaction(List<ItemProfile> itemProfiles, Player player)
        {
            ...
            {
                foreach (var wo in purchaseItems)
                {
                    if (wo.ItemType == ItemType.TinkeringMaterial && DynamicVendorInfo.ItemValues.FindIndex(x => x.Wcid == wo.WeenieClassId) >= 0)
                    {
                        wo.ItemWorkmanship = 5;
                        wo.Structure = 100;
                    }
                ...
                var trx = new List<DynamicVendor.Transaction>();
                foreach (var wcid in from g in purchaseItems where DynamicVendorInfo.ItemValues.FindIndex(x => x.Wcid == g.WeenieClassId) >= 0 select g.WeenieClassId)
                {
            ...
            if (AlternateCurrency != null)
            {
                ...
                if (WeenieClassId == 21747010 /* Vendor is Jiminey */)
                {
                    if (ModdedWeenies.JimineysSalePrices.TryGetValue(item.WeenieClassId, out int specialPrice))
                    {
                        ...
                        if (item.WeenieClassId == 20630) /* MMD */
                            itemValue = (int)Math.Max(1, (uint)Math.Ceiling((1.15f * (itemValue ?? 0)) - 0.1));
                    }
                }

                return (uint)Math.Max(1, itemValue ?? 1);
            }

            if (DynamicVendorInfo != null)
            {
                var curItem = DynamicVendorInfo.ItemValues.Find(x => x.Wcid == item.WeenieClassId);
                if (curItem != null)
                    return GetSellCost((int)curItem.PyrealValue, item.ItemType);
            }

            return GetSellCost(item.Value, item.ItemType);
        }
        ...
        /// <summary>
        /// Simulates the Dynamic Vendor's decaying per-unit sell price for a batch of items being sold to this vendor,
        /// returning the total payout. If this vendor is a Dynamic Vendor, also returns the list of transactions
        /// (one per unit sold) that should be staged against DynamicVendorInfo once the sale is finalized.
        /// </summary>
        private int CalculateDynamicSellPayout(IEnumerable<WorldObject> items, out List<DynamicVendor.Transaction> transactions)
        {
            transactions = null;
            ...
            if (DynamicVendorInfo != null)
            {
                ...
                transactions = new List<DynamicVendor.Transaction>();

                foreach (var item in DynamicVendorInfo.ItemValues.Where(x => items.Select(i => i.WeenieClassId).Contains(x.Wcid)))
            ...
            foreach (var wo in items)
            {
                    ...
                    if ((wo.ItemType == ItemType.TinkeringMaterial && (wo.Structure ?? 0) == 100) /* salvage must be full bag */ || wo.ItemType != ItemType.TinkeringMaterial)
                    {
                        if (tempValueHolder.FindIndex(x => x.Key == wo.WeenieClassId) >= 0)
                        {
                            ...
                            for (int i = 0; i < (wo.StackSize ?? 1); i++) // handle stackables
                            {
                                ...
                                transactions.Add(new DynamicVendor.Transaction(wo.WeenieClassId, tempValueHolder[tempValueHolderIndex].Value, DynamicVendor.Transaction.TransactionType.Sell));
                ...
                payout += GetBuyCost(itemValue, wo.ItemType);
            }
        ...
        {
            return CalculateDynamicSellPayout(items.Values, out _);
        }
        ...
        public void ProcessItemsForPurchase(Player player, Dictionary<uint, WorldObject> items)
        {
            ...
            if (DynamicVendorInfo != null)
            {
                CalculateDynamicSellPayout(items.Values, out var transactions);
                DynamicVendorInfo.AddTransactions(transactions, player.Guid.Full);
                DynamicVendorInfo.CommitTransactions(player.Guid.Full, WeenieClassId);
                DefaultItemsForSale.Clear();
                inventoryloaded = false;
                LoadInventory();
